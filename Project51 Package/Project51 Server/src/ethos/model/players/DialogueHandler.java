 ...
 */
		...
		case 63:
			sendOption5("Resource Area @bla@(@red@Wilderness@bla@) 60K Coins.", "KBD Lair @bla@25K Coins.",
					"Godwars Dungeon + 10 KC @bla@80K Coins.", "Donator Only Rev Cave @bla@10M Coins", "");
			c.dialogueAction = 61;
			break;
		...
		case 14401:
			...
			break;
		case 14402:
			sendOption5("Draynor Rooftop Course", "Varrock Rooftop Course", "Seers Rooftop Course", "Ardougne Rooftop Course", "Back");
		...
		case 14012:
			sendPlayerChat3("Yeah! I've never seen one before.", "I guess it's one small step for NPC's,",
					"one giant leap for Dragonstone!");
			c.nextChat = 14013;
			break;
		case 14013:
...
//Revenant Emblem Trader
		...
		case 645:
			sendNpcChat("You've made it through!", "You have the option to play as an <img=12></img> Iron Man,",
					"<img=13></img> Ultimate Ironman, <img=23></img> Hardcore Iron Man, or Regular.", "Choose from the following interface.");
			c.nextChat = 646;
			break;
		...
		case 650:
			if (c.getMode().isIronman() || c.getMode().isUltimateIronman() || c.getMode().isHardcore()) {
		...
		case 6600:
			sendNpcChat("Hardcore Iron Man accounts have all the restrictions of",
					"regular Iron man accounts with an added 7% drop rate",
		...
		case 661:
			...
			EquipmentSet set = c.getMode().isIronman() ? EquipmentSet.IRON_MAN_ARMOUR
					: c.getMode().isUltimateIronman() ? EquipmentSet.ULTIMATE_IRON_MAN_ARMOUR
							: c.getMode().isHardcore() ? EquipmentSet.HARDCORE_ARMOUR: null;
		...
		case 673:
			sendNpcChat("Glad to see you have joined Dragonstone!", "Let me show you around!",
					"To start, this is our lovely home area.");
			c.nextChat = 674;
			break;

		case 674:
			c.getPA().movePlayer(3079, 3501, 0);
			c.getItems().wearItem(21018, 1, c.playerHat);
			c.getItems().wearItem(21021, 1, c.playerChest);
			c.getItems().wearItem(21024, 1, c.playerLegs);
			c.getItems().wearItem(22555, 1, c.playerWeapon);
			c.getItems().wearItem(19720, 1, c.playerAmulet);
			c.getItems().wearItem(21776, 1, c.playerCape);
			c.getItems().wearItem(13235, 1, c.playerFeet);
			c.getItems().wearItem(12825, 1, c.playerShield);
			c.getItems().wearItem(19544, 1, c.playerHands);
		...
		case 677:
			...
			c.getItems().wearItem(22638, 1, c.playerHat);
			c.getItems().wearItem(22641, 1, c.playerChest);
			c.getItems().wearItem(22644, 1, c.playerLegs);
			c.getItems().wearItem(22550, 1, c.playerWeapon);
			c.getItems().wearItem(12018, 1, c.playerAmulet);
			c.getItems().wearItem(21898, 1, c.playerCape);
			c.getItems().wearItem(13237, 1, c.playerFeet);
			c.getItems().wearItem(-1, -1, c.playerShield);
			c.getItems().wearItem(21816, 1, c.playerHands);
		...
		case 6777:
			...
			break;
		case 676:
			c.getPA().movePlayer(3077, 3510, 0);
			c.getItems().wearItem(23075, 1, c.playerHat);
			c.getItems().wearItem(22327, 1, c.playerChest);
			c.getItems().wearItem(22328, 1, c.playerLegs);
			c.getItems().wearItem(22325, 1, c.playerWeapon);
			c.getItems().wearItem(20366, 1, c.playerAmulet);
			c.getItems().wearItem(21285, 1, c.playerCape);
			c.getItems().wearItem(13239, 1, c.playerFeet);
			c.getItems().wearItem(7462, 1, c.playerHands);
			c.getItems().wearItem(-1, -1, c.playerShield);
			sendNpcChat("You can start training Slayer by talking", "to any of these Slayer Masters. Slayer can be",
					"very profitable and has some great rewards!");
			c.nextChat = 677;
			break;
		...
		case 679:
			c.getPA().movePlayer(2910, 2725, 0);
			sendNpcChat("Here you can talk to Nurse BouBou who will", "replenish your health, special attack,",
					" and run whenever you need it.");
			c.nextChat = 680;
		case 680:
			c.getPA().movePlayer(3089, 3467, 0);
			sendNpcChat("On the south side of the home area", "you will find many early game skilling,",
					"options for some of the most useful skills");
			c.nextChat = 681;
			break;
		case 681:
			c.getPA().movePlayer(3098, 3506, 0);
		...
		case 900:
			sendNpcChat4("Hello! Would you like to enable Daily Tasks?", "These tasks can be assigned and completed",
					"once a day for some pretty neat rewards!", "Enable Daily Tasks?", 7371, "Daily Task Master");
			c.nextChat = 901;
			break;
		case 901:
