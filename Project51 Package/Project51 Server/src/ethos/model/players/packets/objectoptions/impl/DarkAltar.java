...
public class DarkAltar {
	...
	public static void handleDarklightTransaction(Player c) {
		...
		if (c.getItems().playerHasItem(6746, 1) && c.getItems().playerHasItem(19677, 3)) {
			...
			/**
			 * The next two lines are here to ensure players have charges saved to their account, that are then set to 0 before giving players the REAL 1000 charges
			 * Ran into a lot of issues with the charging on initial darklight transformation & in a rush this seemed like the best fix.
			 */
			c.setArcLightCharge(c.getArcLightCharge() + 1);
			c.setArcLightCharge(0);
			//End of precautionary charges being added & deleted
			c.setArcLightCharge(1000);
