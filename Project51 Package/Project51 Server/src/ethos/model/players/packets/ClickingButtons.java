	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (c.getTutorial().isActive()) {
			if(actionButtonId == 132014) {
				c.getPA().sendFrame126("Standard playing style. Enjoy playing Dragonstone with", 33815);
			...
			if(actionButtonId == 132017) {
				if(c.gamemode == 0) {
					...
					c.sendMessage("You have selected normal gamemode! Enjoy Dragonstone!");
				} else if(c.gamemode == 1) {
					c.sendMessage("You have selected Ironman gamemode! Enjoy Dragonstone!");
					c.setMode(Mode.forType(ModeType.IRON_MAN));
					c.getRights().setPrimary(Right.IRONMAN);
				} else {
					c.sendMessage("You have selected Ultimate Ironman gamemode! Enjoy Dragonstone!");
		...
		case 108012:
			if(c.getName().equalsIgnoreCase("KID BUU")) {
				int offset = 9999;
		...
		case 40170:
			...
			if(c.questTab == 3) {
				c.sendMessage("@gre@This quest is coming soon!");
			}
			break;
		case 40171:
			if(c.questTab == 0) {
			...
			if(c.questTab == 2) {
				c.forcedChat("I have donated " + c.amDonated + " USD.");
			}
			if(c.questTab == 3) {
				c.sendMessage("@red@Got an idea for a quest? Pitch it to KID BUU and it could get added in to the game!");
				c.sendMessage("@red@If your idea becomes a real quest you will be rewarded!");
			}
			break;
		case 40172:
			if(c.questTab == 0) {
			...
			if(c.questTab == 3) {
				c.sendMessage("@red@Got an idea for a quest? Suggest it to KID BUU and it could get added in to the game!");
				c.sendMessage("@red@If your idea becomes a real quest you will be rewarded!");			}
			break;
		case 40173:
			if(c.questTab == 0) {
		...
		case 40177:
			if(c.questTab == 0) {
				c.getPA().sendFrame126("https://discord.gg/Ch9jpd8", 12000);
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getWesternDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I have " + c.dailyPoints + " Daily Task Points.");
			}
			if(c.questTab == 3) {
		...
		case 40178:
			if(c.questTab == 0) {
				c.getPA().sendFrame126("http://YOURLINKHERE.everythingrs.com/services/vote", 12000);
			} 
			if(c.questTab == 1) {
				c.getDiaryManager().getWildernessDiary().display();
			}
			if(c.questTab == 2) {
				c.forcedChat("I have " + c.raidPoints + "  Raid Points.");
			}
			if(c.questTab == 3) {
