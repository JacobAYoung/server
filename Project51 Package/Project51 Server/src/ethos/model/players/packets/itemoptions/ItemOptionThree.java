...
public class ItemOptionThree implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		case 21347:
			...
			break;
		case 21816:
			if (c.absorptionToggle == true) {
				c.sendMessage("@pur@You have disabled Ether absorption!@pur@");
				c.absorptionToggle = false;
				return;
			}
			else {
			c.absorptionToggle = true;
			c.sendMessage("@pur@You have enabled Ether absorption!@pur@");
			}
			break;
		case 21817:
			if (c.getItems().freeSlots() < 2) {
				c.sendMessage("You need at least 2 inventory spaces to do this!");
				return;
			}
			c.getDH().sendDialogues(9051, 778);
			break;
		case 21221:
		case 22118:
			if (c.inWild()) {
		...
		case 12926:
			...
			break;
		case 22323:
			int sanguincharge =  c.getSangStaffCharge();
			if (c.getSangStaffCharge() <= 0) {
				c.sendMessage("@red@You have no charges left in your staff.");
				return;
			}
			if (c.getSangStaffCharge() >= 1) {
				c.getItems().addItem(565, 9 * sanguincharge);
				c.setSangStaffCharge(0);
				c.sendMessage("@red@You uncharge the staff.");
			}
			break;
		case 2552:
		case 2554:
