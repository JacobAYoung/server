    ...
    public void processPacket(Player c, int packetType, int packetSize) {
		...
		case 3847:
			if (Server.getMultiplayerSessionListener().inAnySession(c)) {
				...
				c.getPA().sendFrame126("@dre@This information can change at anytime:", 8145);
				
				c.getPA().sendFrame126("Slayer is a good money making skill", 8148);
				c.getPA().sendFrame126("The Daily Point store has many useful rewards", 8149);
				c.getPA().sendFrame126("Ring of Charos can teleport you to your slayer task", 8150);
				c.getPA().sendFrame126("You can imbue Black Masks as well as Slayer Helmets", 8151);
				c.getPA().sendFrame126("You can use the ::help command to easily report bugs", 8152);
				c.getPA().sendFrame126("Skillcapes have many useful perks", 8153);
				c.getPA().sendFrame126("Max capes give players the perks from ALL skill capes", 8154);
				c.getPA().sendFrame126("Revenant Caves are good for money & Slayer", 8155);
				c.getPA().sendFrame126("If you use ::stuck YOU CAN NOT MOVE, TYPE, OR CLICK", 8156);
				c.getPA().sendFrame126("after typing the command. This will stop it from working", 8157);
