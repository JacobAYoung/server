...
public class Player extends Entity {
	...
	public void highscores() {
		getPA().sendFrame126("ServerNameHere - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void destruct() {
		...
		disconnected = true;
		if(!getRights().isOrInherits(Right.OWNER) && !getRights().isOrInherits(Right.ADMINISTRATOR) && !getRights().isOrInherits(Right.GAME_DEVELOPER)) {
			if(getMode().isUltimateIronman()) {
				com.everythingrs.hiscores.Hiscores.update("34yOj45xuz6SAQVXwdpFCdzUQLYjSese9Msetz1YVuKa17g2JcbWVF0obHgn0ViRGlOOIklz", "Ultimate Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
			if (getMode().isHardcore()) {
				com.everythingrs.hiscores.Hiscores.update("34yOj45xuz6SAQVXwdpFCdzUQLYjSese9Msetz1YVuKa17g2JcbWVF0obHgn0ViRGlOOIklz", "Hardcore Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
			} else {
				if(getMode().isIronman()) {
					com.everythingrs.hiscores.Hiscores.update("34yOj45xuz6SAQVXwdpFCdzUQLYjSese9Msetz1YVuKa17g2JcbWVF0obHgn0ViRGlOOIklz", "Ironman", this.playerName, this.playerRank, this.playerXP, debugMessage);
				}
				com.everythingrs.hiscores.Hiscores.update("34yOj45xuz6SAQVXwdpFCdzUQLYjSese9Msetz1YVuKa17g2JcbWVF0obHgn0ViRGlOOIklz", "Normal Mode", this.playerName, this.playerRank, this.playerXP, debugMessage);
			}
		}
	...
	public void initialize() {
		try {
			...
			sendMessage("@dre@Update: Raids & Rev Caves expansions, New home, and a ton of Bug Fixes!");
			sendMessage("@dre@Be sure to check out all the new content!");
			...
			if(getRights().isOrInherits(Right.MODERATOR) || getRights().isOrInherits(Right.HELPER) ||
				...
				for (Right right : Right.values()) {
					if(right == getRights().getPrimary())
						if(!playerName.equalsIgnoreCase("Kid Buu"))
	...
	public void updateRedTab() {
		...
		getPA().sendFrame126("@or1@ Amount Donated: @gre@" + this.amDonated, 10411);
		...
		getPA().sendFrame126("@or1@ Daily Points = @gre@" +this.dailyPoints, 10417);
		getPA().sendFrame126("@or1@ Quest Points = @gre@" + this.questPoints, 10418);
	...
	public void updatePurpleTab() {
		...
		getPA().sendFrame126(" The first quest is coming soon!", 10410);
		getPA().sendFrame126(" Got an idea for a quest?", 10411);
		getPA().sendFrame126(" Suggest it to Kenny!", 10412);
		getPA().sendFrame126(" Theres a good chance it'll get added!", 10413);
	...
	public int[] IMBUED_SLAYER_HELMETS = { 11865, 19641, 19645, 19649, 21266, 21890, 23075 };
	...
	public void updateRank() {
		...
		if (amDonated >= 10 && amDonated < 50) {
			if (getRights().isOrInherits(Right.IRONMAN) || getRights().isOrInherits(Right.ULTIMATE_IRONMAN) || getRights().isOrInherits(Right.HARDCORE) || getRights().isOrInherits(Right.OSRS) || getRights().isOrInherits(Right.HELPER) || getRights().isOrInherits(Right.MODERATOR)) {
