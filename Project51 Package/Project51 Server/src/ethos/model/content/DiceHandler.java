...
public class DiceHandler {
	...
	public static final Boundary DICING_AREA = new Boundary(2872, 2685, 2950, 2753);
		...
		if (System.currentTimeMillis() - c.diceDelay >= 5000) {
			...
			PlayerHandler.nonNullStream()
					...
					.forEach(player -> {
						...
						if (roll < 55) {
							player.sendMessage("[@red@DICE@bla@] @cr18@ The gambler @blu@"
									+ Misc.capitalizeJustFirst(c.playerName) + " @bla@wins.");
						} else {
							player.sendMessage(
									"[@red@DICE@bla@] @cr18@ The gambler @blu@" + Misc.capitalizeJustFirst(c.playerName)
