...
public class CorporealBeast {
	...
	public static void attack(Player attacker, Damage damage) {
		...
		NPC corp = NPCHandler.getNpc(319);
		int change = ((int) (damage.getAmount() / 2));
		int change2 = ((int) (damage.getAmount() * 1.3));
		int doNotChange = ((int) (damage.getAmount()));

		if (corp.getHealth().getAmount() > 50)
		if (!attacker.getItems().isWearingItem(11824)) {
			damage.setAmount(change);
		}
		if (attacker.getItems().isWearingItem(22610)) {
			damage.setAmount(change2);
		}
		if (attacker.getItems().isWearingItem(13081) || attacker.getItems().isWearingItem(13092)) {
			damage.setAmount(doNotChange);
		}
	}
}
